...
v0.3.6 was verified against production on Claude Code on 2026-09-18 with backend v0.95.10
(the traffic three-part template, PagerDuty open-is-a-status rule, requests-vs-limits terms
and event scope citation, from a 40-attempt comparison rerun). v0.3.5 was verified against
production on Claude Code on 2026-09-18 with backend v0.95.8
...
```bash
codex plugin marketplace add anyshift-io/agent-plugin --ref v0.3.6
...
The last two commands are required by Codex CLI 0.147.0 because its plugin installer does not yet
launch OAuth for a plugin-owned remote MCP server: `codex mcp add` registers the endpoint and
`codex mcp login` opens the browser OAuth flow; select the Anyshift project the plugin may read. Start a new Codex thread after installation so the skill and MCP tools
are loaded together.
...
| Claude Code on macOS (v0.3.6 surface, backend v0.95.10) | 2.1.274, same native install | Pass | Pass | Ten tools | Guidance derived from a 40-attempt native-vs-plugin rerun where the plugin arm scored 102/120 coverage against 95/120 native, with the remaining flags all interpretation: traffic verdicts from eligibility, a date-bounded PagerDuty query hiding an open incident, memory limits described as reservations, an OOM event cited without its namespace | 2026-09-18 |
